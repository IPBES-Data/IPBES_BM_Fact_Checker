...
| `R/build_nli_ready_parquet.R` | `nli_ready_parquet` | Split each BM's `bm_description` (and/or `bm_label`) into sentences (fragments < 20 chars dropped), cross-join with the cleaned `premise` (= title + abstract) of every citing work. One row per (work × BM sentence) with `sentence_number`, `sentence_source`, `claim`, `premise`, and token estimates (`abstract_tokens`, `sentence_tokens`, `approx_tokens`). Parallelised over km/bm partitions with `parallel::mclapply` (`workers` from config). Writes to `output/nli_ready/assessment=<id>/km=<km>/bm=<bm>/` |
| `R/build_nli_scores_parquet.R` | `nli_scores_parquet` | **Active NLI scoring.** Consumes `nli_ready_parquet`. `nli.<active>.host` may be a single hostname or a list (pool); health-checked up front (fails loudly listing every unreachable host, stops on cross-host model mismatch). Work is distributed by **claim** — `(km, bm, sentence_source, sentence_number)`, not by BM — via greedy LPT (longest-processing-time-first) balancing on remaining pair count, fanned out with `parallel::mclapply(seq_len(n_hosts), ..., mc.cores = n_hosts)`; a pool of 1 (scalar `host:`) behaves identically to the old single-host path. Stores `label` + `p_supports`/`p_refutes`/`p_nei`/`confidence`/`uncertain` per (work × claim). Rows with `approx_tokens > max_length` are skipped (logged; see NEXT_STEPS.md). **Resumable:** existing scores for the `(nli_config, assessment)` are preserved and only new pairs are appended (dedup key `(km, bm, sentence_number, sentence_source, work_id, claim)`). Writes to `output/nli_scores/nli_config=<cfg>/assessment=<id>/km=<km>/bm=<bm>/claim_id=<claim_id>/` |
