...
| `R/build_nli_bm_explorer.R` | `nli_bm_explorer_html` | Interactive plotly widget per assessment with a BM-select dropdown (default "All BMs") toggling three precomputed panels: label distribution (%), confidence distribution (mean/median in a dynamic annotation), and alignment distribution. `build_nli_bm_explorer()` builds the plotly object (all traces/buttons precomputed in R — the dropdown only toggles `visible`, no client-side recomputation); `save_nli_bm_explorer()` writes it as a self-contained standalone HTML (`htmlwidgets::saveWidget(selfcontained = TRUE)`) to `output/tables/nli_bm_explorer_<id>.html`, embedded in the report via `<iframe>`. Deliberately **not** printed in-place via `cat(knitr::knit_print(widget))` inside the report's `results: asis` loop — that was tried first and found to not propagate the plotly.js dependency in Quarto's HTML output (empty div, no chart); the iframe/saveWidget route matches the existing convention for other interactive artifacts (e.g. `build_overlap_key_paper_table`'s DT tables). |
