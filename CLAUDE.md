...
| `R/build_nli_scores_parquet.R` | `nli_scores_parquet` | **Active NLI scoring.** Consumes `nli_ready_parquet`. Groups by `(sentence_number, sentence_source, claim)` within each (km, bm) so all premises for one claim POST in a single request; stores `label` + `p_supports`/`p_refutes`/`p_nei`/`confidence`/`uncertain` per (work × sentence). Rows with `approx_tokens > max_length` are skipped (logged; see NEXT_STEPS.md). **Resumable:** existing scores for the `(nli_config, assessment)` are preserved and only new pairs are appended (dedup key `(km, bm, sentence_number, sentence_source, work_id, claim)`). Writes to `output/nli_scores/nli_config=<cfg>/assessment=<id>/km=<km>/bm=<bm>/` |
