...
input/config.yaml
    ...
    → output/llm_candidate_scope/assessment=<id>/km=<km>/bm=<bm>/             (per-claim citing-work allow-list, subset: "sm" narrowing input, gitignored)
    → output/llm_verification/scores/llm_config=<cfg>/subset=<subset>/assessment=<id>/km=<km>/bm=<bm>/   (Phase 2 LLM review of REFUTES/uncertain NLI pairs, gitignored)
...
**Active scoring approach — NLI** (`TD_BM_NLI_approach.qmd`): `nli_scores_by_claim`
...
Each `llm_verification` config also picks a `subset` — `"all"` (every citing
work under the claim's BM) or `"sm"` (only citing works tracing back, via
`llm_candidate_scope_parquet`, to a seed reference whose sub-chapter matches
the claim's own evidence-reference braces). Its architecture is ported from
...
Config is split into fine-grained targets (`sparql_url`, `assessments_list`, `nli_active`/`nli_config`, `llm_verification_active`/`llm_verification_config`) so that adding new config sections or changing one entry does not invalidate other targets. Each top-level config key maps to its own intermediate target; `llm_verification:` follows the exact same `active` + named `configs` structure as `nli:` (four configs — `openrouter_cheap`/`openrouter_cheap_sm`/`openrouter_midtier`/`openrouter_toptier_gpt5` — carried over from the sibling `Categorisation_Literature` project's proven `epistemology:` config, including its `max_active: 24`/`max_tokens: 8000` values; each config also sets `subset: "all"` or `"sm"`, see `llm_candidate_scope_parquet` below).
...
| `R/build_llm_candidate_scope_parquet.R` | `llm_candidate_scope_parquet` | **Per-claim candidate scoping** for `subset: "sm"` configs. Chains `refs_parquet`'s `sm` (sub-chapter id, exploded and normalised — lowercased, `box`/`table`/`spm table` prefixes and `(...)` annotations stripped, comma/semicolon-split) to seed `doi` to seed OpenAlex work id (`works_parquet`) to citing work (`output/snowball/edges`, filtered to `to %in%` the in-scope seed ids), matched against each evidence-segmented claim's own brace tokens (`{5.4.1, 5.4.2}`) via `evidence_tokens_match()`'s prefix rule (`"5.4.2.1"` matches `"5.4.2.1.a"`; bare `"4"` matches any `"4.x.y"`). `extract_claim_evidence_tokens()` deliberately **duplicates** (does not call) `segment_bm_by_evidence()` from `build_nli_ready_evidence_parquet.R` — even a behavior-preserving edit to that function would mark `nli_ready_evidence_parquet`, and the NLI scoring chain downstream of it, outdated, since `targets` hashes function bodies as dependencies — verified byte-for-byte identical segment boundaries against the original across several test cases. Cross-checks its own derived `claim_id`s against `nli_ready_evidence_parquet`'s real ones at build time and `warning()`s (not `stop()`s) on any mismatch, guarding against the two implementations silently drifting apart later. A claim with no evidence braces at all is simply absent from the output — the consumer (`build_llm_verification_parquet.R`) treats that as "fall back to unrestricted," never as "exclude." Reads only already-existing, unmodified targets and makes no network/API calls; adding it does not invalidate `download_works.R`/`build_snowball_parquet.R`/`build_works_citing_parquet.R`/`build_nli_ready_evidence_parquet.R`/the NLI scoring chain. Always computed regardless of which `llm_verification` config is active. Writes to `output/llm_candidate_scope/assessment=<id>/km=<km>/bm=<bm>/`. |
| `R/build_llm_verification_parquet.R` | `llm_verification_parquet` | **Phase 2 LLM verification**, one call per assessment (not per claim — no crew/lock dispatch needed since OpenRouter is a shared endpoint, unlike the fixed RunPod pool Phase 1 load-balances across). `select_llm_verification_candidates()` filters Phase 1's scored output per the active config's own `nli_labels` (`label %in% nli_labels`, `NULL`/absent = any) and `nli_certainty` (`"certain"`/`"uncertain"`/`NULL` = any) fields, combined with AND — every shipped config currently sets `nli_labels: [REFUTES]`, `nli_certainty: certain` (the confirmed first step: 26,861 of 3,547,231 scored pairs) — then joins back `premise` text from `nli_ready_evidence_parquet` (Phase 1's own output never stores premise text); when the active config's `subset` is `"sm"`, `apply_candidate_scope()` further narrows against `llm_candidate_scope_parquet` (matched on the full `(km, bm, claim_id)` key, not `claim_id` alone — `claim_id` strings like `"bm_description-01"` are reused across 30-49 different BMs per assessment, so matching on `claim_id` alone let one BM's scope entries wrongly authorize another's candidates until this was caught and fixed), falling back to unrestricted for any claim absent from that scope table. One `ellmer::chat_openrouter()` call per routed `(claim, work)` pair, architecture ported from the sibling `Categorisation_Literature` project's `R/llm_epistemology.R`: resumable per-pair JSON cache under `output/llm_verification/raw/model=<model>/prompt=<hash>/` (hash covers system prompt + user template + output schema, so editing any of them starts a fresh cache namespace — deliberately **not** keyed by `subset`/routing fields, since a cached answer for a given `(model, prompts, schema)` pair is valid regardless of which subset or routed bucket brought it there), a retry pass for transient provider failures, and a fail-loud check if more than half the pairs return unparseable responses (a broken-plumbing signal, not honest abstention). `quote_is_verbatim()` then checks every LLM-cited quote actually occurs (normalised comparison) in the premise text shown to it, demoting `sufficient_evidence`/`llm_label` to `NOT_ENOUGH_INFO` if not — this is the piece most worth having ported rather than reinvented. Writes to `output/llm_verification/scores/llm_config=<config>/subset=<subset>/assessment=<id>/nli_route=<route>/km=<km>/bm=<bm>/`, where `nli_route` is `nli_route_label()`'s hive-safe encoding of **that row's own** NLI outcome (e.g. `REFUTES-certain`), computed per row from its actual label/uncertain values rather than once per config call — so a config listing several labels/certainties (e.g. `nli_labels: [REFUTES, SUPPORTS]`) still writes one `nli_route=` subdirectory per distinct outcome actually present (`nli_route=REFUTES-certain` and `nli_route=SUPPORTS-certain`, never a merged string), and switching `active`, widening routing fields, or comparing several configs side by side never overwrites another combination's scored rows — same reasoning as `nli_config` in Phase 1's own output path. Configured via `input/config.yaml`'s `llm_verification:` block (`active` + named `configs`, mirroring `nli:`). Deliberately **not** a dependency of `report_fact_checker`, so rendering the report never forces an OpenRouter call; its output is not yet merged into `nli_overview_data` — see TD_NLI_LLM_two_phase.qmd's Recommended Workflow. |
...
- `output/nli_scores_evidence/` — active evidence-segmented NLI scores, same schema, partitioned by assessment/km/bm/claim_id
- `output/llm_candidate_scope/` — per-claim citing-work allow-list for `subset: "sm"` configs, partitioned by assessment/km/bm
- `output/llm_verification/raw/` — Phase 2's resumable per-pair JSON cache, partitioned by model/prompt hash
- `output/llm_verification/scores/` — Phase 2 LLM verification of NLI-flagged pairs, partitioned by llm_config/subset/assessment/km/bm
