...
| `R/build_nli_bm_explorer.R` | `nli_bm_explorer_html` | Interactive plotly widget per assessment with **three** linked controls — a BM-select dropdown (default "All BMs"), a minimum-confidence slider (0-0.9, default 0.5), and a table-label dropdown (REFUTES/SUPPORTS/NOT_ENOUGH_INFO, default REFUTES). The BM dropdown + slider jointly pick one precomputed `(bm, threshold)` cell for the three chart panels; all three controls together pick one `(bm, threshold, label)` cell for a 4th element, a drill-down table. All works are always shown in the charts (no filtering); the threshold instead splits every bar into two **stacked** segments via `barmode = "stack"`: confidence-≥-threshold (solid fill, bottom) and confidence-<-threshold (hollow/outline-only via transparent `marker.color` + colored `marker.line`, on top) — label distribution (%) splits each label's bar this way (each bar also labelled `n / N` via `text`/`textposition = "outside"` on the top trace), the confidence-histogram panel splits each bin by its own mid vs threshold (plus a dashed threshold line), and the alignment-histogram panel splits each bin by how many of its works meet the threshold (a genuine cross-tab, computed via two separate `hist()` calls on the above/below subsets sharing the same breaks). Below the three panels (bar-panel `yaxis`/`yaxis2`/`yaxis3` domains compressed to the top ~32% via a post-`subplot()` `domain` override — confirmed safe: unlike `shapes`, this preserves each axis's already-set `title`/`range`), a `type = "table"` trace lists the actual works for the current `(bm, threshold, label)` — `work_id`, confidence, alignment — sorted by confidence descending and capped at 50 rows (the true match count is always shown in the caption, never silently truncated). Every `(bm, threshold)` cell has 6 chart traces and every `(bm, threshold, label)` cell has 1 table trace, all precomputed once in R; all three controls are `method = "skip"` (native dropdown/slider UI + active-index tracking, no automatic restyle) and a small `htmlwidgets::onRender()` JS handler reads all three current indices from the widget's own layout and applies one combined `Plotly.restyle`/`Plotly.relayout` — this avoids the controls resetting each other, which plotly's declarative per-button/per-step `args` can't do on their own. Two plotly/htmlwidgets quirks were found and fixed by isolated repro rather than assumption: (1) `plotly::subplot()` silently drops a `shapes` layout key even after `plotly_build()` (annotations survive the identical call) — the confidence-threshold marker line is therefore set **only** via the JS `Plotly.relayout` path, never via `plotly::layout(shapes = ...)`; (2) calling `add_trace()` repeatedly on an already-`subplot()`-merged figure (as opposed to a fresh `plot_ly()` object) leaves each new trace's internal plotly "attrs" bookkeeping entry unnamed, which crashes `htmlwidgets::saveWidget()`'s `shouldEval()` validation once mixed with the properly-named bar-trace attrs at realistic scale (30+ BMs) — the table traces are therefore built on their own fresh `plot_ly()` object and its `$x$data`/`$x$attrs` spliced directly into the merged figure (dropping the fresh object's own phantom placeholder attrs entry, `[-1]`, which would otherwise silently shift every table trace's built index by one). Verified end-to-end against a real headless-Chrome DOM (`chromote`) at full production scale (30 BMs, ~1.9M rows, 2790 traces): all three controls combine correctly (changing any one preserves the other two), marker fill/outline styling and n/N counts are exactly as intended, above+below segment values sum back to the known unfiltered label percentages, the table shows the correct capped/sorted rows with an accurate caption, and a screenshot confirmed no layout overlap between the controls, annotations, panel titles, charts, table caption, and table. `save_nli_bm_explorer()` writes the figure as a self-contained standalone HTML (`htmlwidgets::saveWidget(selfcontained = TRUE)`) to `output/tables/nli_bm_explorer_<id>.html`, embedded in the report via `<iframe>` — deliberately **not** printed in-place via `cat(knitr::knit_print(widget))` inside the report's `results: asis` loop, which was tried first and found to not propagate the plotly.js dependency under Quarto's HTML output (empty div, no chart); the iframe/saveWidget route matches the existing convention for other interactive artifacts (e.g. `build_overlap_key_paper_table`'s DT tables). |
