...
for (data_path in nli_overview_data_paths) {
  ...
  cat(
    "Select a background message from the dropdown to see its label ",
    "distribution, confidence distribution (mean/median reported above the ",
    "panel), and alignment-score distribution — or leave **All BMs** ",
    "selected for the assessment-wide picture.\n\n"
  )
  ...
  if (length(explorer_this)) {
    cat(sprintf(
      '<iframe src="%s" width="100%%" height="560px" style="border:none;"></iframe>\n\n',
      explorer_this
    ))
  }
