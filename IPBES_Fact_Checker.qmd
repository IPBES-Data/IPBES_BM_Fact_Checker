...
report_files_dir <- paste0(
  tools::file_path_sans_ext(knitr::current_input()), "_files"
)
...
# wrappers (each just this report's format/layout plus
# `{{< include TD_<name>.md >}}`) and copied into the report's own _files/
# sidecar so the links below work in a distributed copy of the report too.
...
- [TD_LLM_approach](`r td_doc_link("TD_LLM_approach")`) — the parked LLM-comparison approach
- [TD_NLI_LLM_two_phase](`r td_doc_link("TD_NLI_LLM_two_phase")`) — the two-phase NLI → LLM design (**planned, not yet implemented**)
