...
nli_bm_explorer_paths <- targets::tar_read(nli_bm_explorer_html)
```
...
for (data_path in nli_overview_data_paths) {
  ...
  cat(
    ...
    "from the slider to see the label distribution, confidence distribution ",
    "(dashed line marks the threshold; mean/median reported above the ",
    "panel), and alignment-score distribution for works scored at or above ",
    "that confidence — or leave **All BMs** / confidence 0 selected for the ",
    "unfiltered, assessment-wide picture.\n\n"
  )
  ...
  if (length(explorer_this)) {
    cat(sprintf(
      '<iframe src="%s" width="100%%" height="600px" style="border:none;"></iframe>\n\n',
      explorer_this
    ))
  }
