...
A companion per-assessment report narrows REFUTES-labelled citing works
down through NLI certainty and LLM re-verification. See each report's own
Methodology section for the full 3-level sieve definition.

```{r}
#| label: refutes_funnel_links
#| results: asis
...
refutes_funnel_html_paths <- targets::tar_read(report_refutes_funnel_html)
for (p in refutes_funnel_html_paths) {
  id <- sub("^IPBES_REFUTES_Report_(.*)\\.html$", "\\1", basename(p))
  cat(sprintf("- [%s REFUTES Funnel Report](%s)\n", id, basename(p)))
}
```
