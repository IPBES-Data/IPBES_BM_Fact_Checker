...
(`bm_split_report_table`/`bm_split_report_html` targets) renders one table
per assessment, per whichever granularity is currently active, showing
every original BM field alongside every resulting claim (and, where
applicable, its extracted confidence qualifier) — generated directly from
