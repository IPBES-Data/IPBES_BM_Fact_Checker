...
**Status note:** written while `MoritzLaurer/deberta-v3-large-zeroshot-v2.0`
...
"Status update" and the `nli:` comment in `input/config.yaml`) — this
document's fine-tuning design was never executed against either model, and
would need re-targeting to `bge-m3-zeroshot-v2.0-c` (a different base
architecture) before being acted on. Kept as the original design record
below, un-rewritten.
