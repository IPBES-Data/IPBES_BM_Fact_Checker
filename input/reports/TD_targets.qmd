...
**Phase 2 LLM verification** (`llm_verification_parquet`) reviews whichever slice of NLI scores each config's `nli_labels`/`nli_certainty` fields select (currently `REFUTES` + `SUPPORTS`, both `certain`, on every shipped config — ~79k of 3.5M scored pairs), one `ellmer::chat_openrouter()` call per `(claim, work)` pair, optionally narrowed further by `llm_candidate_scope_parquet` (per-claim citing-work scoping via IPBES's own sub-chapter evidence references, `subset: "default"` — not the active cost lever right now, `nli_labels`/`nli_certainty` are doing that job instead) — see [TD_NLI_LLM_two_phase.qmd](TD_NLI_LLM_two_phase.html) for the design and [R/build_llm_verification_parquet.R](R/build_llm_verification_parquet.R)/[R/build_llm_candidate_scope_parquet.R](R/build_llm_candidate_scope_parquet.R) for the implementation. The earlier truth/citing-document LLM design it replaced (`prompts_truth_parquet` → `prompts_citing_parquet` → `alignement_scores_parquet`) has been removed; see [TD_LLM_approach.qmd](TD_LLM_approach.html) for that record.
...
regardless of which `llm_verification` config is active; `subset: "all"`
configs simply never read its output.
...
claim, an allow-list of citing works tied to that claim's own evidentiary
sub-chapter rather than the whole BM's — the input `llm_verification_parquet`'s
`subset: "default"` configs (this value used to be called `"sm"`) narrow
against. Supported for `naive_bm`/`atomic_bm`; a no-op sentinel for
`complete_bm` (no per-sub-claim evidence tokens exist for whole-field
claims). Reads only already-existing, unmodified targets
...
`subset` is `"default"` (not the active cost lever right now — see
[TD_NLI_LLM_two_phase.qmd](TD_NLI_LLM_two_phase.html#candidate-scoping)).
Resumable per-pair JSON cache under
...
only `openrouter_cheap_sm` uses `subset: "default"`), the model tiers carried
