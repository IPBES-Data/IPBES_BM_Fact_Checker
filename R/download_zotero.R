infer_zotero_group_id <- function(refs_path) {
  ...
  if (any(!grepl("^[0-9]+$", group_ids))) {
    ...
    stop("Could not parse Zotero group id from refs$zotero: ",
         paste(head(bad_urls, 5), collapse = ", "))
  }

  if (length(group_ids) != 1L) {
    stop("Expected exactly one Zotero group id in refs$zotero, found: ",
...
zotero_item_to_row <- function(item, group_id) {
  ...
  get_last_name <- function(c) {
    if (!is.null(c$lastName) && nchar(c$lastName) > 0) c$lastName
    else if (!is.null(c$name)    && nchar(c$name)     > 0) c$name
    else NA_character_
  }

  first_author <- if (length(creators) > 0) get_last_name(creators[[1]]) else NA_character_
  ...
  chr <- function(x) if (is.null(x) || length(x) == 0L) NA_character_ else as.character(x[[1L]])
  ...
  data.frame(
    ...
    title        = chr(d$title),
    authors      = if (!is.na(authors) && nchar(authors) > 0) authors else NA_character_,
    first_author = first_author,
    year         = year_str,
    doi          = chr(d$DOI),
    abstract     = chr(d$abstractNote),
    zotero_url   = paste0("https://www.zotero.org/groups/", group_id, "/items/", chr(d$key)),
    stringsAsFactors = FALSE
  )
}

download_zotero <- function(assessment, refs_path, output_root = "output/zotero") {
  ...
  for (i in seq_along(starts)) {
    start <- starts[[i]]
    page  <- i
    message("  Fetching items ", start + 1L, "-", min(start + limit, total), " of ", total)
    ...
    items <- jsonlite::fromJSON(httr2::resp_body_string(resp), simplifyVector = FALSE)
    if (!length(items)) {
      next
    }
  ...
  message("Wrote ", total_rows, " rows to ", output_path, " across ", length(starts), " pages")
